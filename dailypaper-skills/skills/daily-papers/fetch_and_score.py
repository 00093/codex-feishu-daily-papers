...
from user_config import daily_papers_config, daily_papers_dir, history_file_path
...
def score_paper(paper: dict, is_trending: bool = False) -> int:
    ...
    # The user wants embodied intelligence / VLA / RL / world-model papers,
    # especially combinations. Generic RL without a robot/embodied/world-model
    # context is too broad for this feed.
    if not (embodied_hits or world_model_hits or (rl_hits and control_hits)):
        return -999

    # Keep generic world-model/RL papers only when they are not obviously outside
    # the user's embodied/VLA/robotics focus.
    off_domain = any(
        kw in text
        for kw in [
            "atmospheric",
            ...
            "strategic trajectory abstraction",
            "cooperative multi-agent reinforcement learning",
            "operator-guided invariance",
        ]
    )
    if off_domain and not embodied_hits:
        return -999
    ...
    # 3.5. Cross-topic bonus: prefer papers combining embodied/VLA/robotics with
    # world models, RL, or policy learning over isolated vision/language papers.
    combo_count = sum([embodied_hits, world_model_hits, rl_hits, control_hits])
    if combo_count >= 3:
        score += 4
    elif embodied_hits and (world_model_hits or rl_hits or control_hits):
        score += 2
    ...
    has_relevance = keyword_hits > 0 or embodied_hits or world_model_hits or rl_hits
    if is_trending:
